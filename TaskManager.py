...
conn = mysql.connect(host="localhost", user="root", password="/your password/")
cur = conn.cursor()
...
def addTask():
    ...
    if (
        ...
        cur.execute(
            f"insert into tasks(task,date,stat) values('{task}','{date[0]}/{date[1]}/{date[2]}','notcompleted');"
        )
...
def completed():
    ...
    for i in range(len(list(tasksDict.keys()))):
        if list(tasksDict.values())[i] != 0:
            cur.execute(
                f"update tasks set stat='completed' where id={int(list(tasksDict.keys())[i][3:])}"
            )
            conn.commit()
    getTask()
    ProgressBar.set(len(tasksCom) / (len(tasksCom) + len(tasksNotcom)))
    ProgressPcent.configure(
        text=len(tasksCom) / (len(tasksCom) + len(tasksNotcom)) * 100
    )
...
def renderTask(res):
    for i in res:
        if i[3] != "completed":
            globals()[f"tsk{i[0]}"] = tk.CTkCheckBox(
                ContentFrame, text=i[1], onvalue=i[0], offvalue=0
            )
...
def getTask():
    ...
    if len(date) != 1:
        if (
            ...
            date = formatDate(date)
            cur.execute(
                f"select * from tasks where date='{date[0]}/{date[1]}/{date[2]}'"
            )
            res = cur.fetchall()
            renderTask(res)
    else:
...
StatFrame.pack(side="right", anchor="e", padx=10)

ProgFrame = tk.CTkFrame(TaskFrame, border_width=0)
ProgFrame.pack(side="top", anchor="s", fill="x", pady=10)
CompleteButton = tk.CTkButton(
    ProgFrame,
    text="Complete/Check progress",
    fg_color="#2b405f",
    corner_radius=18,
    border_width=0,
    state="disabled",
...
TaskText = tk.CTkTextbox(
    EntryFrame, corner_radius=10, bg_color="#293840", height=80, width=700
)
TaskText.pack(side="top", anchor="w", fill="x")
DateEntry = tk.CTkEntry(
    EntryFrame,
    corner_radius=10,
    bg_color="#293840",
    width=700,
